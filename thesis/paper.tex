...
\newcommand{\code}[1]{\footnotesize\texttt{#1}\normalsize} % layout of in-line code snippets
...
this were given in the previous section was in \ref{ex:simpleembed} and
\ref{ex:whembed}. The verb used in both example was \example{know}. The meaning
postulate for \example{know} is the following:
...
already drew upon the this statement, postulate \ref{def:know} puts this into a
...
would be wrong, and one would only \emph{believe to know}.

The proposition $snorts(Vincent)$, which would evaluate to a truth value, is
turned into a \wh-construct by \example{whether} -- the exact circumstances of
