...
:- use_module(intensionalExampleModels,[example/2]).
...
satisfy(Formula,model(D,Ws,A),G,pos):-
   nonvar(Formula),
   Formula = some(X,SubFormula),
   var(X),
   memberList(V,D),
   satisfy(SubFormula,model(D,Ws,A),[g(X,V)|G],pos).

satisfy(Formula,model(D,Ws,A),G,neg):-
   nonvar(Formula),
   Formula = some(X,SubFormula),
   var(X),
   ...
   setof(V,
         (
          memberList(V,D),
          satisfy(SubFormula,model(D,Ws,A),[g(X,V)|G],neg)
         ),
         All).

satisfy(Formula,model(D,Ws,A),G,undef):- 
   nonvar(Formula),
   Formula = some(X,SubFormula),
   (
    nonvar(X)
   ;
    var(X),
    memberList(V,D), 
    satisfy(SubFormula,model(D,Ws,A),[g(X,V)|G],undef)
   ). 
...
/*========================================================================
   One-place extensional predicates
========================================================================*/
...
satisfy(Formula,model(D,Ws,A),G,pos) :-
	...
	, \+ Symbol = not
	, i(Argument,model(D,Ws,A),G,Value)
	, Ws = [(w0,world(F))|_]
...
/*========================================================================
   Two-place extensional predicates
========================================================================*/
...
satisfy(Formula,model(D,Ws,A),G,pos):-
	...
	, i(Arg1,model(D,Ws,A),G,Value1)
	, i(Arg2,model(D,Ws,A),G,Value2)
	, Ws = [(w0,world(F))|_]
...
satisfy(Formula,model(D,Ws,A),G,undef):-
	...
	;
		var(Arg1)
		, \+ i(Arg1,model(D,Ws,A),G,_)
	; 
		var(Arg2)
		, \+ i(Arg2,model(D,Ws,A),G,_)
	;
		atom(Arg1)
		, \+ i(Arg1,model(D,Ws,A),G,_)
	;
...
/*========================================================================
   Extensional Interpretation of Constants and Variables
========================================================================*/

i(X,model(D,Ws,_A),G,Value):-
	Ws = [(w0,world(F))|_]
	,
	(
		var(X)
...
% Predicates that deal with possible and accessible worlds.

answer(protoq(X,Formula),model(D,M),G,Answers) :-
	var(X)
.
